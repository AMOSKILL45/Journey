...
**Phase 1.5 — Stripe Identity / passport verification** ✅ DONE (`create-identity-session` + `stripe-identity-webhook` edge fns). Deferred: Apple/Google Sign-In (Phase 1.5) · real overworld pixel art (Phase 8) · asset tasks (CC0 audio, reaction/stamp pixel-art sprites). (Phase 3 T2 lat/lng generated cols verified applied; 6C sound infra shipped.)
