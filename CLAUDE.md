...
**Phase 9 — Social Foundation** ✅ COMPLETE (9A–9D) — decomposed + built via a multi-agent workflow (seed → 3 parallel client modules → verify), backend + audits inline. **Most security-sensitive phase** (opens private data to public read → private-by-default + strict scoped opt-in). Discovered the social columns already existed (`trips`/`profiles`, Phase 1) and `trips` SELECT already allowed public read. **9A** path public-read via a single `milestones` SELECT policy (ADR-008 allowlist — only milestones, never docs/checklists/locations/members) + `VisibilityControl` (private/unlisted/public_view + confirm dialog + copy-link). **9B** read-only public trip view at `(public)/trip/[token]` via the `journey://t/{token}` deep link (ADR-009 — reuses `share_token`; anonymous HTML web page deferred to v1.1); shows trip + path only. **9C — fixed a pre-existing HIGH PII leak**: `profiles` SELECT was `true` (any authenticated user could read everyone's phone/passport/legal-name/stripe) → tightened to own-only + `get_trip_member_profiles`/`get_public_profile` column-limited SECURITY DEFINER RPCs (ADR-007, grant-hardened); `listMembers` repointed off the nested `profiles(...)` embed onto the member RPC; public-profile screen + opt-in toggle. **9D** empty v1.1 schema (`trip_join_requests`/`reports`/`user_blocks`/`trip_discovery_index` + RLS, no UI). **100% OTA** (existing scheme; `expo-clipboard` lazy-guarded → copy degrades to an Alert until the next native build). **1338 tests passing (1267→1338).** Advisors baseline-clean; **PII fix verified via synthetic RLS (a stranger sees 0 profile rows)**; security review (inline) no HIGH/MED. Specs: `…2026-06-05-journey-phase-9-social-foundation-design.md` + `…-architecture-adr.md` (ADR-007/008/009); Plan: `…plans/2026-06-05-journey-phase-9-social-foundation.md`; UI: `…reference/phase-9-ui-spec.md`. Device verification (EAS build) needed for the deep-link open + real clipboard; backends live. **v1.1 follow-ups**: discovery feed / join flow / reports-blocks UI + the anonymous HTML public page + universal links (native).

**Phase 10 — Stores Polish & Submit-Readiness** ✅ CODE COMPLETE (10A–10E) — full pipe (brainstorm → ADR-010/011/012 → ui-ux-pro-max → multi-agent workflow [seed → 6 parallel: onboarding · account/legal · 4 feature-lot passes] → runtime-contracts → code-validator → security review). **10A** first-run onboarding: `@features/onboarding` 4-screen skippable carousel + `PrePermissionSheet` priming (notifications+location, fail-open) + first-run gate in `app/index.tsx` (flag `onboarding.flags.v1`). **10B** `@shared` `EmptyState`/`LoadingState`/`ErrorState` rolled across ~14 screens (sprite+sentence+1 action). **10C** a11y: **Readable Mode** = `useReadableMode` (manual ∨ `getFontScale()≥1.5`) + `PixelText` bascule pixel→Fredoka 700 (**already-bundled font → OTA-safe, ADR-011**) + `A11ySettings` + accessibilityLabels + accent-as-text contrast fixes. **10D** i18n audit: `keyParity` (en⟷fr) + `runtimeContracts` used-key test. **10E** legal gates: **in-app account deletion** (`delete-account` edge fn verify_jwt + `purge_account_data` SECURITY DEFINER, grant-hardened revoke-from-PUBLIC) — **reserved ghost user `de1e7e00-0000-4000-8000-000000000000` ("Former traveller")** anonymizes shared content, transfers owned shared trips to the oldest remaining member, deletes sole-owner trips + personal rows (**ADR-010**, grounded in live FK introspection — naive `auth.users` delete would cascade-nuke shared trips / be blocked by NOT NULL NO ACTION FKs); **GDPR export** (`export-account-data` → JSON → share sheet); age gate; privacy/ToS links (`@core/env`). **100% OTA, zero new native deps.** **1606 tests (1338→1606).** Advisors baseline-clean (`purge_account_data` correctly absent from SECURITY-DEFINER-executable lists). Account-deletion **verified via synthetic SQL** (owner-transfer + anonymize-to-ghost + solo-delete + membership-detach, rolled back) + **security review clean** (no IDOR — uid from JWT only; no cross-user export; purge grant-hardened). Runtime contracts (`src/core/__tests__/runtimeContracts.test.ts`): i18n used-keys, edge-fn-name↔dir, **ghost-UUID consistency client↔migration↔edge-fn** (drift fails CI), onboarding route, env. Spec: `…2026-06-06-journey-phase-10-stores-polish-design.md` + `…-architecture-adr.md`; UI: `…reference/phase-10-ui-spec.md`; checklist: `…reference/phase-10-runtime-contracts-checklist.md`. **Reste = asset task** (icons 1024/splash/screenshots FR+EN/feature graphic = pixel-art artist) + device verification (next EAS build: first-run gate, perm prompts, Readable Mode @150%, real delete/export round-trips) + host privacy/ToS text & set the 2 EAS env URLs.
