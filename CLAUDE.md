...
**Phase 4 — Documents + Checklist + Smart Reminders** — decomposed into 5 sub-projects (4A Documents · 4B Checklists · 4C Push infra · 4D Smart reminders trip · 4E Personal/life reminders). **Phase 4A — Documents** ✅ DONE (11/11 + contract tests + security fix). **Phase 4B — Checklists** ✅ DONE (14/14 — full scope + contract tests + security audit clean). **Phase 4C — Push infra** ✅ DONE (Max/notifications hub — send_push edge fn deployed + contract tests + security audit clean; needs EAS build to test push on device).
...
**Phase 1.5 — Stripe Identity / passport verification** ✅ DONE (`create-identity-session` + `stripe-identity-webhook` edge fns). Deferred: Phase 3 T2 apply_migration (queued for next Supabase MCP session) · Apple/Google Sign-In (Phase 1.5) · sounds (Phase 6) · real overworld pixel art (Phase 8).
